...
export default function ProductCard({
  ...
  return (
    ...
    >
      <div className="bg-card h-32 md:h-36 lg:h-40 flex items-center justify-center p-4">
        {img ? (
          <Image
            ...
            width={160}
            height={130}
            className="object-contain max-h-32.5 shadow-[0_4px_12px_rgba(0,0,0,0.15)]"
          />
        ) : (
      ...
      <div className="p-3">
        <div className="font-sans-condensed font-medium text-2xs uppercase text-brand mb-1">
          {category}
        </div>
      ...
      <div className="border-t border-zinc-100 px-3.5 py-2 flex justify-between items-center mt-auto">
        {badge && (
          <span className="bg-brand text-white font-sans-condensed font-bold text-2xs uppercase px-2 py-0.5">
            {badge}
          </span>
        )}
        <span className="font-sans-condensed text-button-md text-brand font-medium ml-auto">
          Ver mais ›
        </span>
      </div>
    </Link>
