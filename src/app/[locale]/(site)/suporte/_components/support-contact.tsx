...
export async function SupportContact({ contact, contactInfo }: Readonly<SupportContactProps>) {
  const t = await getTranslations('Support.contact')

  const infoItems = contactInfo
    ? ([
        { key: 'phone', label: t('contactInfoPhone'), value: contactInfo.phone },
        { key: 'email', label: t('contactInfoEmail'), value: contactInfo.email },
        { key: 'whatsapp', label: t('contactInfoWhatsapp'), value: contactInfo.whatsapp },
      ] as const)
    : []
