import type { Category, Product, ProductBlock, Subcategory } from '@/lib/api/contracts'
import {
  CATALOG_CATEGORIES,
...
const CATEGORY_NAMES: Record<string, LocaleMap> = {
  'cat-amplificadores': { 'pt-BR': 'Amplificadores', en: 'Amplifiers', es: 'Amplificadores' },
  'cat-processadores': { 'pt-BR': 'Processadores', en: 'Processors', es: 'Procesadores' },
  'cat-crossovers': { 'pt-BR': 'Crossovers', en: 'Crossovers', es: 'Crossovers' },
  'cat-controles': { 'pt-BR': 'Controles', en: 'Controllers', es: 'Controles' },
  'cat-fontes-carregadores': { 'pt-BR': 'Fontes e carregadores', en: 'Power Supplies', es: 'Fuentes y cargadores' },
  'cat-mesas-de-som': { 'pt-BR': 'Mesas de som', en: 'Mixers', es: 'Mesas de sonido' },
  'cat-acessorios': { 'pt-BR': 'Acessórios', en: 'Accessories', es: 'Accesorios' },
  'cat-subwoofers': { 'pt-BR': 'Subwoofers', en: 'Subwoofers', es: 'Subwoofers' },
}

const SUBCATEGORY_NAMES: Record<string, LocaleMap> = {
  'sub-full-range': { 'pt-BR': 'Full Range', en: 'Full Range', es: 'Full Range' },
  ...
  'sub-automotiva': { 'pt-BR': 'Fonte Automotiva', en: 'Automotive Power Supply', es: 'Fuente Automotiva' },
}

export function getCatalogCategoriesForLocale(locale?: string): Category[] {
  return CATALOG_CATEGORIES.map((cat) => ({
    ...cat,
    name: CATEGORY_NAMES[cat.id] ? pick(CATEGORY_NAMES[cat.id], locale) : cat.name,
  }))
}

export function getCatalogSubcategoriesForLocale(locale?: string): Subcategory[] {
  return CATALOG_SUBCATEGORIES.map((sub) => ({
    ...sub,
    name: SUBCATEGORY_NAMES[sub.id] ? pick(SUBCATEGORY_NAMES[sub.id], locale) : sub.name,
  }))
}
...
const PRODUCT_DESCRIPTIONS: Record<string, LocaleMap> = {
  'prod-st-4000eq-4c': {
    'pt-BR': 'Amplificador 4 canais com alta eficiência para sistemas automotivos de média e alta potência.',
  ...
  'prod-st-800-4-compact': {
    'pt-BR': 'Compacto, eficiente e ideal para projetos que priorizam espaço interno.',
  ...
  'prod-st-1200-1d-bass': {
    'pt-BR': 'Projeto focado em graves com resposta rápida e controle térmico aprimorado.',
  ...
  'prod-stx-96-pro': {
    'pt-BR': 'Processador digital com presets avançados e equalização em tempo real.',
  ...
  'prod-volcano-12k': {
    'pt-BR': 'Subwoofer de alta excursão para projetos SPL com grande deslocamento de ar.',
  ...
  'prod-sf-120a': {
    'pt-BR': 'Fonte chaveada para sistemas automotivos de alta demanda contínua.',
  ...
  'prod-st-350-4-mini': {
    'pt-BR': 'Modelo compacto legado, mantido para histórico e suporte documental.',
  ...
  'prod-ex-3000eq-bass': {
    'pt-BR': 'Módulo mono de alta corrente para projetos SPL com subwoofers de grande deslocamento.',
  ...
  'prod-ir-400-4': {
    'pt-BR': 'Amplificador full-range para sistemas compactos com excelente controle de médio e agudo.',
  ...
  'prod-bravo-1600-1': {
    'pt-BR': 'Projetado para graves encorpados com estabilidade térmica para uso contínuo.',
  ...
  'prod-cl-2500-digital': {
    'pt-BR': 'Linha CL com DSP de ajuste fino para alta fidelidade em sistemas multivia.',
  ...
  'prod-stx-2448bt': {
    'pt-BR': 'Processador digital com conectividade Bluetooth e interface de ajuste em tempo real.',
  ...
  'prod-stx-3248-pro': {
    'pt-BR': 'Processador para projetos profissionais com alinhamento de tempo e crossover avançado.',
  ...
  'prod-vulcan-15k': {
    'pt-BR': 'Subwoofer de alto desempenho para competições, com bobina reforçada e resposta rápida.',
  ...
  'prod-sf-200a-pro': {
    'pt-BR': 'Fonte automotiva para sistemas de alta potência com proteção completa contra sobrecarga.',
  ...
  'prod-rca-pro-series': {
    'pt-BR': 'Cabos RCA blindados para reduzir ruído e preservar dinâmica em sistemas de alta fidelidade.',
  ...
  'prod-kit-anl-200a': {
    'pt-BR': 'Kit de fusível ANL e porta-fusível para instalações seguras de alta corrente.',
...
const BLOCK_CONTENT: Record<string, BlockContentMap> = {
  'blk-4000eq-image': {
    caption: { 'pt-BR': 'Design robusto para alta entrega de potência.', en: 'Robust design for high power delivery.', es: 'Diseño robusto para alta entrega de potencia.' },
  },
  'blk-4000eq-text': {
    title: { 'pt-BR': 'Performance sem limite', en: 'Unlimited Performance', es: 'Rendimiento sin límites' },
    content: {
      'pt-BR': 'Projetado para instalações exigentes, o ST-4000EQ entrega estabilidade e controle térmico.',
  ...
  'blk-4000eq-video': {
    title: { 'pt-BR': 'ST-4000EQ 4 Canais em ação', en: 'ST-4000EQ 4 Channels in action', es: 'ST-4000EQ 4 Canales en acción' },
    description: {
  ...
  'blk-2000eq-image': {
    caption: { 'pt-BR': 'Módulo mono para graves.', en: 'Mono module for bass.', es: 'Módulo mono para graves.' },
  },
  'blk-8004-text': {
    title: { 'pt-BR': 'Compacto e inteligente', en: 'Compact and smart', es: 'Compacto e inteligente' },
    content: {
  ...
  'blk-1200-video': {
    title: { 'pt-BR': 'Setup de graves com ST-1200.1D', en: 'Bass setup with ST-1200.1D', es: 'Setup de graves con ST-1200.1D' },
    description: {
  ...
  'blk-stx-html': {
    html: {
      'pt-BR': '<div><strong>DSP 96kHz</strong> com presets customizáveis.</div>',
  ...
  'blk-volcano-image': {
    caption: { 'pt-BR': 'Subwoofer de alto deslocamento.', en: 'High-displacement subwoofer.', es: 'Subwoofer de alto desplazamiento.' },
  },
  'blk-fonte-text': {
    title: { 'pt-BR': 'Energia estável', en: 'Stable power', es: 'Energía estable' },
    content: {
  ...
  'blk-350-mini-text': {
    title: { 'pt-BR': 'Modelo legado', en: 'Legacy model', es: 'Modelo legado' },
    content: {
  ...
  'blk-ex3000-image': {
    caption: { 'pt-BR': 'Módulo mono de alta corrente para projetos SPL.', en: 'High-current mono module for SPL builds.', es: 'Módulo mono de alta corriente para proyectos SPL.' },
  },
  'blk-ir400-image': {
    caption: { 'pt-BR': 'Full-range compacto para controle de médio e agudo.', en: 'Compact full-range for midrange and treble control.', es: 'Full-range compacto para control de medios y agudos.' },
  },
  'blk-bravo-text': {
    title: { 'pt-BR': 'Graves encorpados', en: 'Rich bass', es: 'Graves pronunciados' },
    content: {
      'pt-BR': 'Projetado para uso contínuo com estabilidade térmica e controle preciso de frequência.',
  ...
  'blk-cl2500-image': {
    caption: { 'pt-BR': 'Linha CL com DSP de ajuste fino para alta fidelidade.', en: 'CL line with fine-tuned DSP for high fidelity.', es: 'Línea CL con DSP de ajuste fino para alta fidelidad.' },
  },
  'blk-stx2448-text': {
    title: { 'pt-BR': 'Conectividade sem fio', en: 'Wireless connectivity', es: 'Conectividad inalámbrica' },
    content: {
      'pt-BR': 'Interface Bluetooth para ajuste em tempo real diretamente do smartphone.',
  ...
  'blk-stx3248-html': {
    html: {
      'pt-BR': '<div><strong>DSP 96kHz</strong> com alinhamento de tempo, crossover avançado e 8 entradas / 12 saídas configuráveis.</div>',
  ...
  'blk-vulcan15-image': {
    caption: { 'pt-BR': 'Subwoofer de alto desempenho para competições.', en: 'High-performance subwoofer for competitions.', es: 'Subwoofer de alto rendimiento para competiciones.' },
  },
  'blk-sf200-text': {
    title: { 'pt-BR': 'Alta corrente', en: 'High current', es: 'Alta corriente' },
    content: {
      'pt-BR': 'Fonte automotiva com 200A contínuos e proteção completa contra sobrecarga e curto-circuito.',
  ...
  'blk-rca-text': {
    ...
    content: {
      'pt-BR': 'Blindagem dupla que elimina ruído e preserva a dinâmica original do sistema de áudio.',
  ...
  'blk-anl-text': {
    title: { 'pt-BR': 'Proteção essencial', en: 'Essential protection', es: 'Protección esencial' },
    content: {
      'pt-BR': 'Kit fusível ANL 200A com porta-fusível em liga de cobre para instalações seguras de alta corrente.',
