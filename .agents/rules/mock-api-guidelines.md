---
description: 'Use when maintaining the offline local mock fallback. Mocks are auto-fallback when CMS_API_BASE_URL is unset and must stay contract-compatible.'
applyTo: 'src/lib/mock/**/*'
---

# Mock API Guidelines

## Scope

Develop and PR previews use the dedicated API. Maintain this fallback only for
offline local work and contract verification; do not add mock-only features or
expand its surface.

## Two Patterns Coexist

| Pattern | Used for | Type source |
|---|---|---|
| Mock provider (`createMockCmsProvider`) | Public site RSC pages (no auth needed) | Orval models in `src/api/stetsom/model/` |
| Orval React Query hooks + BFF | Admin/CMS client components | Orval models in `src/api/stetsom/model/` |

Both patterns share the same Orval-generated types as their contract.

## Mock Fallback (public site)

- Active when `CMS_API_BASE_URL` is unset (default for local dev without backend)
- Fixtures: `src/lib/mock/*.ts` → consumed by `src/lib/api/providers/mock-provider.ts`
- Mock payloads must conform to Orval-generated types — import from `@/api/stetsom/model`

## Data Quality Rules

- Use realistic values: UUIDs, ISO 8601 dates, canonical slugs, valid URLs
- Multilingual fields must use `I18nString` format: `{ pt: '...', en?: '...', es?: '...' }` (key is `pt`, not `pt-BR`)
- `ProductBlock` combos: include TEXT/IMAGE/VIDEO where applicable with unique sequential `order`
- `ProductFile`: keep `version`, `is_active`, and `type` semantics coherent
- Never introduce mock-only fields in UI code paths — if a field is not in the Orval model, don't use it
- Use mocks as a compatibility safety net, not the primary source of behavior

## Generated Mock Data

- `src/lib/mock/data.json` is auto-generated by `pnpm mock:dump`.
  **Never edit it manually.** Any manual edit will be lost on the next sync.
- The dump script fetches from the real API (`CMS_API_BASE_URL`) and replaces
  the fixture atomically so removed records and routes cannot survive a sync.
- To change mock structure: edit the API/database first, then run
  `pnpm mock:dump` to synchronize.

### Mock Dispatch

Two paths resolve mock data from URL segments:

1. **Custom handlers** (`handlers/index.ts`): registered for keys like
   `products` — provide custom filtering, sorting, and pagination logic
   matching upstream behavior
2. **Raw data.json lookup** (`loader.ts`): for all other keys —
   `segments.join("--")` maps URL path to a data.json key
   (e.g. `/api/pages/home/cms` → `pages--home--cms`)
