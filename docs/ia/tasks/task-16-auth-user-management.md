...
- [ ] `POST /api/auth/login` com credenciais corretas seta cookie `admin_token` httpOnly e retorna `AuthPayload`
- [ ] `POST /api/auth/login` com credenciais erradas retorna 401 com `INVALID_CREDENTIALS`
- [ ] Acessar qualquer rota `/admin/*` sem cookie → redireciona para `/admin/login`
- [ ] Após login bem-sucedido → redireciona para `/admin`
- [ ] `POST /api/auth/logout` limpa o cookie e redireciona para `/admin/login`
- [ ] `/admin/usuarios` exibe tabela com os 3 usuários mock (nome, email, role, status, último login)
- [ ] Botão "Novo usuário" abre modal com formulário (nome, e-mail, senha, role, ativo)
- [ ] Salvar novo usuário → aparece na tabela (mock in-memory)
- [ ] Botão "Editar" abre modal preenchido com os dados do usuário
- [ ] Desativar/ativar usuário altera o badge de status na tabela sem reload
